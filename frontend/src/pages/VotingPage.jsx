...
function VotingPage() {
  ...
  const shareUrl = `${window.location.origin}/view/${pollId}`;
  ...
  const handleCopyLink = async () => {
    try {
      ...
      console.error(err);
      toast.error("Failed to copy link. You can copy it manually.");
    } finally {
      setIsCopying(false);
    }
  };
  ...
  return (
    <div className="bg-base-200 min-h-screen p-6 text-white flex flex-col items-center">
      ...
      <div className="w-full max-w-lg mb-4">
        <div className="bg-base-100 rounded-lg p-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2 overflow-hidden">
            <FaLink className="hidden md:block" />
            <span className="text-xs md:text-sm truncate">{shareUrl}</span>
          </div>
          <button
            className="btn btn-xs md:btn-sm btn-primary mt-2 md:mt-0"
